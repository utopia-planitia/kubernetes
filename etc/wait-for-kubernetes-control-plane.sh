...
for ID in $NODES; do
  echo wait for kubelet on node${ID}
...
for ID in $ETCD; do
  echo wait for etcd${ID} health check
...
for ID in $NODES; do
  echo wait for node${ID} port 6443
...
for ID in $NODES; do
  echo wait for node${ID} registration
...
for ID in $NODES; do
  echo wait for node${ID} kube-proxy
